...
function App() {
  ...
  function adjust(entry: TimeEntry) { setTimeEditor({ entry, kind: 'worked', day: localDateValue(entry.clock_in), startTime: localTimeValue(entry.clock_in), endTime: entry.clock_out ? localTimeValue(entry.clock_out) : '', breakMinutes: String(Math.round((entry.paused_seconds ?? 0) / 60)), breakStart: '12:00', breakEnd: '12:30', jobId: entry.job_id }); }
  ...
  return <main className="app-shell"><header><div className="header-brand"><img className="header-logo" src="/pwa-icon-3.png" alt="RidgePoint" /><span>Jobs &amp; Clock</span></div><div className="user"><strong>{profile.full_name}</strong><button className="text-button" onClick={signOut}>Sign out</button></div></header><nav>{(['jobs', 'clock', 'timesheet'] as const).map((item) => <button key={item} className={tab === item ? 'active' : ''} onClick={() => setTab(item)}>{item === 'jobs' ? 'Jobs' : item === 'clock' ? 'Time clock' : 'Timesheet'}</button>)}</nav>{notice && <div className="notice">{notice}<button onClick={() => setNotice('')}>Dismiss</button></div>}
    ...
    {printMode && <section className="print-sheet"><div className="preview-bar"><strong>Print-ready preview</strong><button className="text-button" onClick={() => setPrintMode(null)}>Close preview</button></div><img className="print-logo" src="/ridgepoint-remodeling-logo.png" alt="RidgePoint Remodeling" />{printMode === 'jobs' ? <><h1>Hours by job - {profile.full_name}</h1><p>{formatDate(start)} - {formatDate(end)} | Two-week pay period</p><div className="print-total"><strong>{hours.toFixed(2)}</strong><span>Total hours across all jobs</span></div><h2>Job-hour summary</h2>{jobTotals.map((job) => <div className="print-row" key={job.title}><span>{job.title}</span><b>{job.hours.toFixed(2)} h</b></div>)}</> : <><h1>Timesheet - {profile.full_name}</h1><p>{formatDate(start)} - {formatDate(end)} | Two-week pay period</p><div className="print-total"><strong>{hours.toFixed(2)}</strong><span>Total hours</span></div><div className="print-grid"><span>Regular hours</span><b>{regularHours.toFixed(2)}</b><span>Overtime hours</span><b>{overtimeHours.toFixed(2)}</b><span>Hourly rate</span><b>$35.00</b><span>Overtime rate</span><b>$52.50</b><span>Estimated gross pay</span><b>${grossPay.toFixed(2)}</b></div></>}<p className="print-foot">Generated for payroll review.</p><button className="wide print-action" onClick={() => window.print()}>Print this timesheet</button></section>}
