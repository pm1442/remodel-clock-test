...
function App() {
  ...
  const [tasks, setTasks] = useState<Record<string, { id: string; title: string; color: string; done: boolean }[]>>({});
  ...
  function addTask(jobId: string) { const title = window.prompt('Task note'); if (!title) return; setTasks((current) => ({ ...current, [jobId]: [...(current[jobId] ?? []), { id: String(Date.now()), title, color: 'burgundy', done: false }] })); }
