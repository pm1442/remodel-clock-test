...
function hoursBetween(day: string, startTime: string, endTime: string) { const start = new Date(`${day}T${startTime}:00`); const end = new Date(`${day}T${endTime}:00`); return !Number.isNaN(start.getTime()) && end > start ? (end.getTime() - start.getTime()) / 3_600_000 : 0; }
...
function App() {
  ...
  const [timeEditor, setTimeEditor] = useState<{ entry: TimeEntry | null; day: string; startTime: string; endTime: string; jobId: string } | null>(null);
  ...
  function adjust(entry: TimeEntry) { setTimeEditor({ entry, day: localDateValue(entry.clock_in), startTime: localTimeValue(entry.clock_in), endTime: entry.clock_out ? localTimeValue(entry.clock_out) : '', jobId: entry.job_id }); }
  function addHours() { setTimeEditor({ entry: null, day: start.toISOString().slice(0, 10), startTime: '08:00', endTime: '17:00', jobId: selectedJobId }); }
  async function saveHours(event: FormEvent) { event.preventDefault(); if (!profile || !timeEditor || !timeEditor.jobId || !timeEditor.startTime || !timeEditor.endTime) return setNotice('Choose a job, a start time, and an end time.'); const startAt = new Date(`${timeEditor.day}T${timeEditor.startTime}:00`); const endAt = new Date(`${timeEditor.day}T${timeEditor.endTime}:00`); if (endAt <= startAt) return setNotice('End time must be after start time.'); const query = timeEditor.entry ? supabase.from('time_entries').update({ job_id: timeEditor.jobId, clock_in: startAt.toISOString(), clock_out: endAt.toISOString(), paused_at: null, paused_seconds: 0, edited_at: new Date().toISOString() }).eq('id', timeEditor.entry.id) : supabase.from('time_entries').insert({ company_id: profile.company_id, employee_id: profile.id, job_id: timeEditor.jobId, clock_in: startAt.toISOString(), clock_out: endAt.toISOString(), edited_at: new Date().toISOString() }); const { error } = await query; if (error) setNotice(error.message); else { setTimeEditor(null); await refresh(); } }
