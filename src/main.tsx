...
function App() {
  ...
  return <main className="app-shell"><header><div><div className="wordmark">RIDGEPOINT</div><span>Remodel Clock</span></div><div className="user"><strong>{profile.full_name}</strong><button className="text-button" onClick={signOut}>Sign out</button></div></header><nav>{(['jobs', 'clock', 'timesheet'] as const).map((item) => <button key={item} className={tab === item ? 'active' : ''} onClick={() => setTab(item)}>{item === 'jobs' ? 'Jobs' : item === 'clock' ? 'Time clock' : 'Timesheet'}</button>)}</nav>{notice && <div className="notice">{notice}<button onClick={() => setNotice('')}>Dismiss</button></div>}
    ...
    {tab === 'timesheet' && <section><p className="kicker">CURRENT PAY PERIOD</p><h1>{formatDate(start)} - {formatDate(end)}</h1><p className="period-note">Two weeks, Thursday through Wednesday</p><div className="hours"><strong>{hours.toFixed(2)}</strong><span>Total hours</span></div><div className="overtime-summary"><span>Regular: {regularHours.toFixed(2)} h</span><span>Overtime: {overtimeHours.toFixed(2)} h</span></div><div className="timesheet-actions"><button onClick={addHours}>Add hours for a day</button><button className="ghost compact" onClick={() => setShowJobTotals(!showJobTotals)}>{showJobTotals ? 'Hide job breakdown' : 'View hours by job'}</button></div>{showJobTotals && <div className="job-breakdown"><strong>Hours by job</strong>{jobTotals.map((job) => <div key={job.title}><span>{job.title}</span><b>{job.hours.toFixed(2)} h</b></div>)}</div>}<div className="entries">{periodEntries.length ? periodEntries.map((entry) => <article className="entry" key={entry.id}><div><h2>{jobTitle(entry)}</h2><p>{formatDate(new Date(entry.clock_in))} | {formatTime(entry.clock_in)}{entry.clock_out ? ` - ${formatTime(entry.clock_out)}` : ' - still clocked in'}</p></div><div><strong>{toHours(entry).toFixed(2)} h</strong>{entry.clock_out && <button className="text-button" onClick={() => adjust(entry)}>Adjust hours</button>}</div></article>) : <div className="empty">No time entered in this pay period.</div>}</div><div className="timesheet-actions"><button className="wide ghost" onClick={() => printTimesheet('payroll')}>Print/share payroll timesheet</button><button className="wide ghost" onClick={() => printTimesheet('jobs')}>Print/share with job hours</button></div></section>}
    {printMode && <section className="print-sheet"><div className="preview-bar"><strong>Print-ready preview</strong><button className="text-button" onClick={() => setPrintMode(null)}>Close preview</button></div><div className="wordmark">RIDGEPOINT REMODELING</div><h1>Timesheet - {profile.full_name}</h1><p>{formatDate(start)} - {formatDate(end)} | Two-week pay period</p><div className="print-total"><strong>{hours.toFixed(2)}</strong><span>Total hours</span></div><div className="print-grid"><span>Regular hours</span><b>{regularHours.toFixed(2)}</b><span>Overtime hours</span><b>{overtimeHours.toFixed(2)}</b><span>Hourly rate</span><b>$35.00</b><span>Overtime rate</span><b>$52.50</b><span>Estimated gross pay</span><b>${grossPay.toFixed(2)}</b></div>{printMode === 'jobs' && <><h2>Hours by job</h2>{jobTotals.map((job) => <div className="print-row" key={job.title}><span>{job.title}</span><b>{job.hours.toFixed(2)} h</b></div>)}</>}<p className="print-foot">Generated for payroll review.</p><button className="wide print-action" onClick={() => window.print()}>Print this timesheet</button></section>}
