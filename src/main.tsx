...
function App() {
  ...
  const [timeEditor, setTimeEditor] = useState<{ entry: TimeEntry | null; kind: 'worked' | 'off'; day: string; startTime: string; endTime: string; breakMinutes: string; breakStart: string; breakEnd: string; jobId: string } | null>(null);
  ...
  async function refresh(userId = session?.user.id) {
    ...
    const [{ data: jobData }, { data: entryData }, { data: activeData }, { data: taskData }, { data: attachmentData }, { data: offDayData }, { data: receiptData }] = await Promise.all([
      ...
      supabase.from('time_entries').select('id, employee_id, job_id, clock_in, clock_out, paused_at, paused_seconds, break_start, break_end, jobs(title)').eq('employee_id', userId).order('clock_in', { ascending: false }),
      ...
      supabase.from('time_off_days').select('id, day, status').eq('employee_id', userId).order('day', { ascending: false }),
    ...
    setTasks(taskGroups); setAttachments(attachmentGroups); setJobs(loadedJobs); setEntries((entryData ?? []) as TimeEntry[]); setOffDays((offDayData ?? []) as OffDay[]); setReceipts((receiptData ?? []) as Receipt[]); setActive(loadedActive);
  ...
  function adjust(entry: TimeEntry) { const customBreak = Boolean(entry.break_start && entry.break_end); setTimeEditor({ entry, kind: 'worked', day: localDateValue(entry.clock_in), startTime: localTimeValue(entry.clock_in), endTime: entry.clock_out ? localTimeValue(entry.clock_out) : '', breakMinutes: customBreak ? 'custom' : String(Math.round((entry.paused_seconds ?? 0) / 60)), breakStart: customBreak ? breakTimeValue(entry.break_start) : '12:00', breakEnd: customBreak ? breakTimeValue(entry.break_end) : '12:30', jobId: entry.job_id }); }
  function addHours() { setTimeEditor({ entry: null, kind: 'worked', day: start.toISOString().slice(0, 10), startTime: '08:00', endTime: '17:00', breakMinutes: '30', breakStart: '12:00', breakEnd: '12:30', jobId: selectedJobId }); }
  ...
  async function saveHours(event: FormEvent) { event.preventDefault(); if (!profile || !timeEditor) return; if (timeEditor.kind === 'off') { const { error } = await supabase.from('time_off_days').upsert({ company_id: profile.company_id, employee_id: profile.id, day: timeEditor.day, status: 'off' }, { onConflict: 'employee_id,day' }); if (error) setNotice(error.message); else { setTimeEditor(null); await refresh(); } return; } if (!timeEditor.jobId || !timeEditor.startTime || !timeEditor.endTime) return setNotice('Choose a job, a start time, and an end time.'); const startAt = new Date(`${timeEditor.day}T${timeEditor.startTime}:00`); const endAt = new Date(`${timeEditor.day}T${timeEditor.endTime}:00`); const customBreak = timeEditor.breakMinutes === 'custom'; const breakMinutes = customBreak ? minutesBetween(timeEditor.day, timeEditor.breakStart, timeEditor.breakEnd) : Number(timeEditor.breakMinutes); const breakStartAt = new Date(`${timeEditor.day}T${timeEditor.breakStart}:00`); const breakEndAt = new Date(`${timeEditor.day}T${timeEditor.breakEnd}:00`); if (endAt <= startAt) return setNotice('End time must be after start time.'); if (customBreak && (breakMinutes <= 0 || breakStartAt < startAt || breakEndAt > endAt)) return setNotice('Your break must fall inside the work shift.'); if (breakMinutes < 0 || breakMinutes * 60_000 >= endAt.getTime() - startAt.getTime()) return setNotice('Break time must be shorter than the shift.'); const values = { job_id: timeEditor.jobId, clock_in: startAt.toISOString(), clock_out: endAt.toISOString(), paused_at: null, paused_seconds: Math.round(breakMinutes * 60), break_start: customBreak ? timeEditor.breakStart : null, break_end: customBreak ? timeEditor.breakEnd : null, edited_at: new Date().toISOString() }; const query = timeEditor.entry ? supabase.from('time_entries').update(values).eq('id', timeEditor.entry.id) : supabase.from('time_entries').insert({ company_id: profile.company_id, employee_id: profile.id, ...values }); const { error } = await query; if (error) setNotice(error.message); else { setTimeEditor(null); await refresh(); } }
  ...
  async function uploadReceipt(file: File | undefined) {
    ...
    if (uploadError) { setUploadingReceipt(false); return setNotice(uploadError.message); }
  ...
  function openAccount() {
    ...
    setAccountCode(''); setAccountCodeSent(false); setAccountVerified(false); setShowAccount(true);
  }
  async function sendAccountCode() {
    ...
    if (error) setNotice(error.message); else { setAccountCodeSent(true); setNotice(`A one-time code was sent to ${email}.`); }
  }
  async function verifyAccountCode(event: FormEvent) {
    event.preventDefault();
    const email = session?.user.email;
    if (!email || !accountCode.trim()) return;
    setAccountBusy(true);
    const { data, error } = await supabase.auth.verifyOtp({ email, token: accountCode.trim(), type: 'recovery' });
    setAccountBusy(false);
    if (error || !data.session) return setNotice(error?.message ?? 'That code could not be verified.');
    setSession(data.session); setAccountVerified(true); setNotice('Code verified. You can now update this account.');
  }
  ...
  return <main className="app-shell"><header><div className="header-brand"><img className="header-logo" src="/pwa-icon-3.png" alt="RidgePoint" /><span>Jobs &amp; Clock</span></div><div className="user"><strong>{profile.full_name}</strong><button className="text-button" onClick={openAccount}>Account</button><button className="text-button" onClick={signOut}>Sign out</button></div></header><nav>{(['jobs', 'clock', 'timesheet', 'receipts', 'assistant'] as const).map((item) => <button key={item} className={tab === item ? 'active' : ''} onClick={() => setTab(item)}>{item === 'jobs' ? 'Jobs' : item === 'clock' ? 'Time clock' : item === 'timesheet' ? 'Timesheet' : item === 'receipts' ? 'Receipts' : 'Ask RidgePoint'}</button>)}</nav>{notice && <div className="notice">{notice}<button onClick={() => setNotice('')}>Dismiss</button></div>}
        ...
        <div className="print-header">
          <div className="print-brand">
            ...
            <div>
              ...
              <p className="print-period">{formatDate(start)} - {formatDate(end)} <span>•</span> Two-week pay period</p>
            </div>
          </div>
          <div className="print-meta">
            <span>Submitted</span><strong>{formatFullDate(new Date())}</strong>
            <span>Company</span><strong>RidgePoint Remodeling</strong>
            <span>Contact</span><strong>(386) 291-5200</strong><strong>jason@ridgepointremodeling.com</strong><strong>8831 SW 88th Ct, Lake Butler, FL 32054</strong>
          </div>
        </div>
        {printMode === 'jobs' ? <>
          <div className="print-total"><strong>{hours.toFixed(2)}</strong><span>Total hours across all jobs</span></div>
          ...
          <div className="print-total"><strong>{hours.toFixed(2)}</strong><span>Total hours</span></div>
          <div className="print-grid"><span>Regular hours</span><b>{regularHours.toFixed(2)}</b><span>Overtime hours</span><b>{overtimeHours.toFixed(2)}</b><span>Hourly rate</span><b>$35.00</b><span>Overtime rate</span><b>$52.50</b><span>Estimated gross pay</span><b>${grossPay.toFixed(2)}</b></div>
        </>}
    ...
    {timeEditor && <div className="modal"><form className="modal-card" onSubmit={saveHours}><h2>{timeEditor.entry ? 'Adjust hours' : 'Add time'}</h2><label>Day<input type="date" value={timeEditor.day} min={start.toISOString().slice(0, 10)} max={end.toISOString().slice(0, 10)} onChange={(event) => setTimeEditor({ ...timeEditor, day: event.target.value })} required /></label>{!timeEditor.entry && <label>Day type<select value={timeEditor.kind} onChange={(event) => setTimeEditor({ ...timeEditor, kind: event.target.value as 'worked' | 'off' })}><option value="worked">Worked</option><option value="off">Off</option></select></label>}{timeEditor.kind === 'off' ? <p className="off-day-note">This day will appear as Off in the current pay period. No work hours will be added.</p> : <><label>Job<select value={timeEditor.jobId} onChange={(event) => setTimeEditor({ ...timeEditor, jobId: event.target.value })}><option value="">Choose a job</option>{jobs.map((job) => <option key={job.id} value={job.id}>{job.title}</option>)}</select></label><div className="time-fields"><label>Start time<input type="time" value={timeEditor.startTime} onChange={(event) => setTimeEditor({ ...timeEditor, startTime: event.target.value })} required /></label><label>End time<input type="time" value={timeEditor.endTime} onChange={(event) => setTimeEditor({ ...timeEditor, endTime: event.target.value })} required /></label></div><label>Break<select value={timeEditor.breakMinutes} onChange={(event) => setTimeEditor({ ...timeEditor, breakMinutes: event.target.value })}><option value="0">No break</option><option value="15">15 minutes</option><option value="30">30 minutes - lunch</option><option value="45">45 minutes</option><option value="60">1 hour</option><option value="custom">Custom time range</option></select></label>{timeEditor.breakMinutes === 'custom' && <div className="time-fields break-range"><label>Break starts<input type="time" value={timeEditor.breakStart} onChange={(event) => setTimeEditor({ ...timeEditor, breakStart: event.target.value })} required /></label><label>Break ends<input type="time" value={timeEditor.breakEnd} onChange={(event) => setTimeEditor({ ...timeEditor, breakEnd: event.target.value })} required /></label></div>}<p className="break-summary">Lunch deducted: <strong>{timeEditor.breakMinutes === 'custom' ? `${timeEditor.breakStart} - ${timeEditor.breakEnd} (${Math.round(minutesBetween(timeEditor.day, timeEditor.breakStart, timeEditor.breakEnd))} min)` : `${timeEditor.breakMinutes} min`}</strong></p><p className="time-calculation">Calculated hours: <strong>{hoursBetween(timeEditor.day, timeEditor.startTime, timeEditor.endTime, timeEditor.breakMinutes === 'custom' ? minutesBetween(timeEditor.day, timeEditor.breakStart, timeEditor.breakEnd) : Number(timeEditor.breakMinutes)).toFixed(2)} h</strong></p></>}<div className="modal-actions"><button type="button" className="ghost" onClick={() => setTimeEditor(null)}>Cancel</button><button>{timeEditor.kind === 'off' ? 'Save Off day' : 'Save hours'}</button></div></form></div>}
    {showAccount && <div className="modal"><section className="modal-card account-card"><div><p className="kicker">ACCOUNT SECURITY</p><h2>Update sign-in</h2><p className="account-helper">We first verify the email already connected to this account.</p></div>{!accountCodeSent ? <button disabled={accountBusy} onClick={() => void sendAccountCode()}>{accountBusy ? 'Sending code...' : 'Send one-time code'}</button> : !accountVerified ? <form onSubmit={verifyAccountCode}><label>Code from your email<input inputMode="numeric" autoComplete="one-time-code" value={accountCode} onChange={(event) => setAccountCode(event.target.value)} placeholder="6-digit code" required /></label><div className="modal-actions"><button type="button" className="ghost" disabled={accountBusy} onClick={() => void sendAccountCode()}>Send again</button><button disabled={accountBusy}>{accountBusy ? 'Verifying...' : 'Verify code'}</button></div></form> : <form onSubmit={saveAccount}><label>Display name<input value={accountForm.fullName} onChange={(event) => setAccountForm({ ...accountForm, fullName: event.target.value })} required /></label><label>New sign-in email<input type="email" value={accountForm.email} onChange={(event) => setAccountForm({ ...accountForm, email: event.target.value })} required /></label><label>New password <small>Leave blank to keep the current password.</small><input type="password" autoComplete="new-password" value={accountForm.password} onChange={(event) => setAccountForm({ ...accountForm, password: event.target.value })} /></label><label>Confirm new password<input type="password" autoComplete="new-password" value={accountForm.confirmPassword} onChange={(event) => setAccountForm({ ...accountForm, confirmPassword: event.target.value })} /></label><div className="modal-actions"><button type="button" className="ghost" onClick={() => setShowAccount(false)}>Cancel</button><button disabled={accountBusy}>{accountBusy ? 'Saving...' : 'Save account'}</button></div></form>}<button className="text-button account-close" onClick={() => setShowAccount(false)}>Close</button></section></div>}
