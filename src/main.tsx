    ...
    {printMode && <section className="print-sheet">
      ...
      <div className="print-document" ref={printDocumentRef}>
        ...
        {printMode === 'jobs' ? <>
          ...
          <div className="print-time-table"><div className="print-time-head"><span>Date</span><span>Time worked</span><span>Break</span><span>Hours</span></div>{dailyEntries.length ? dailyEntries.map((day) => <div className="print-time-row" key={day.day}><div><strong>{formatDate(new Date(`${day.day}T12:00:00`))}</strong><small>{formatWeekday(new Date(`${day.day}T12:00:00`))}</small></div><div>{day.entries.map((entry) => <span key={entry.id}>{formatTime(entry.clock_in)} - {entry.clock_out ? formatTime(entry.clock_out) : 'Open'}</span>)}</div><div>{Math.round(day.entries.reduce((total, entry) => total + (entry.paused_seconds ?? 0), 0) / 60)} min</div><b>{day.hours.toFixed(2)}</b></div>) : <p className="print-empty">No worked hours entered for this pay period.</p>}</div>
