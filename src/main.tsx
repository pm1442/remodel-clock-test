...
function App() {
  ...
  const [timeEditor, setTimeEditor] = useState<{ entry: TimeEntry | null; day: string; startTime: string; endTime: string; breakMinutes: string; jobId: string } | null>(null);
  ...
  function adjust(entry: TimeEntry) { setTimeEditor({ entry, day: localDateValue(entry.clock_in), startTime: localTimeValue(entry.clock_in), endTime: entry.clock_out ? localTimeValue(entry.clock_out) : '', breakMinutes: String(Math.round((entry.paused_seconds ?? 0) / 60)), jobId: entry.job_id }); }
  function addHours() { setTimeEditor({ entry: null, day: start.toISOString().slice(0, 10), startTime: '08:00', endTime: '17:00', breakMinutes: '30', jobId: selectedJobId }); }
  async function saveHours(event: FormEvent) { event.preventDefault(); if (!profile || !timeEditor || !timeEditor.jobId || !timeEditor.startTime || !timeEditor.endTime) return setNotice('Choose a job, a start time, and an end time.'); const startAt = new Date(`${timeEditor.day}T${timeEditor.startTime}:00`); const endAt = new Date(`${timeEditor.day}T${timeEditor.endTime}:00`); const breakMinutes = Number(timeEditor.breakMinutes); if (endAt <= startAt) return setNotice('End time must be after start time.'); if (breakMinutes < 0 || breakMinutes * 60_000 >= endAt.getTime() - startAt.getTime()) return setNotice('Break time must be shorter than the shift.'); const values = { job_id: timeEditor.jobId, clock_in: startAt.toISOString(), clock_out: endAt.toISOString(), paused_at: null, paused_seconds: breakMinutes * 60, edited_at: new Date().toISOString() }; const query = timeEditor.entry ? supabase.from('time_entries').update(values).eq('id', timeEditor.entry.id) : supabase.from('time_entries').insert({ company_id: profile.company_id, employee_id: profile.id, ...values }); const { error } = await query; if (error) setNotice(error.message); else { setTimeEditor(null); await refresh(); } }
  ...
  return <main className="app-shell"><header><div className="header-brand"><img className="header-logo" src="/pwa-icon.png" alt="RidgePoint" /><span>Jobs &amp; Clock</span></div><div className="user"><strong>{profile.full_name}</strong><button className="text-button" onClick={signOut}>Sign out</button></div></header><nav>{(['jobs', 'clock', 'timesheet'] as const).map((item) => <button key={item} className={tab === item ? 'active' : ''} onClick={() => setTab(item)}>{item === 'jobs' ? 'Jobs' : item === 'clock' ? 'Time clock' : 'Timesheet'}</button>)}</nav>{notice && <div className="notice">{notice}<button onClick={() => setNotice('')}>Dismiss</button></div>}
    {tab === 'jobs' && <section>
      {openedJobId ? <>
