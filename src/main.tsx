...
function App() {
  ...
  async function refresh(userId = session?.user.id) {
    ...
    const [{ data: jobData }, { data: entryData }, { data: activeData }, { data: taskData }, { data: attachmentData }, { data: offDayData }] = await Promise.all([
    ...
    setTasks(taskGroups); setAttachments(attachmentGroups); setJobs(loadedJobs); setEntries((entryData ?? []) as TimeEntry[]); setOffDays((offDayData ?? []) as OffDay[]); setActive(loadedActive);
