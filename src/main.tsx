    ...
    {printMode && <section className="print-sheet">
      ...
      <div className="print-document" ref={printDocumentRef}>
        ...
        {printMode === 'jobs' ? <>
          ...
          <div className="print-time-table"><div className="print-time-head"><span>Date</span><span>Time worked</span><span>Job</span><span>Break</span><span>Hours</span></div>{printableDays.length ? printableDays.map((day) => <div className={`print-time-row ${day.off ? 'print-off-row' : ''}`} key={day.day}><div><strong>{formatDate(new Date(`${day.day}T12:00:00`))}</strong><small>{formatWeekday(new Date(`${day.day}T12:00:00`))}</small></div><div className="print-time-values">{day.off ? <span>Off</span> : day.entries.map((entry) => <span key={entry.id}>{formatTime(entry.clock_in)} - {entry.clock_out ? formatTime(entry.clock_out) : 'Open'}</span>)}</div><div className="print-job-values">{day.off ? <span>-</span> : day.entries.map((entry) => <span key={entry.id}>{jobTitle(entry)}</span>)}</div><div className="print-break-values">{day.off ? '-' : `${Math.round(day.entries.reduce((total, entry) => total + (entry.paused_seconds ?? 0), 0) / 60)} min`}</div><b>{day.off ? 'Off' : day.hours.toFixed(2)}</b></div>) : <p className="print-empty">No worked hours or off days entered for this pay period.</p>}</div>
